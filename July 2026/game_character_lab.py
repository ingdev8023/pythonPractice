class GameCharacter:
    ...
    def health(self, new_health):
        if new_health < 0 or (self._health + new_health) < 0:
            self._health = 0
        elif new_health > 100 or (self._health + new_health) > 100:
            self._health = 100
        else:
    ...
    def mana(self, new_mana):
        if new_mana < 0 or (self._mana + new_mana) < 0:
            self._mana = 0
        elif new_mana > 50 or (self._mana + new_mana) > 50:
            self._mana = 50
        else:
            self._mana = new_mana
